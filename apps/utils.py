...
class PyClickMerchantAPIView(APIView):
    ...
    def post(self, request):
        ...
        METHODS = {PREPARE: self.prepare, COMPLETE: self.complete}
    ...
    def click_webhook_errors(
        ...
        created_sign_string = "{}{}{}{}{}{}{}{}".format(
            click_trans_id,
            service_id,
            settings.CLICK_SETTINGS["secret_key"],
            merchant_trans_id,
            merchant_prepare_id,
            amount,
            action,
        ...
        if action not in [PREPARE, COMPLETE]:
            return {"error": ACTION_NOT_FOUND, "error_note": _("Action not found")}

        order = cls.order_load(merchant_trans_id)
        if not order:
            return {"error": ORDER_NOT_FOUND, "error_note": _("Order not found")}
        ...
        if order.status == ClickTransaction.CONFIRMED:
            return {"error": ALREADY_PAID, "error_note": _("Already paid")}
        ...
        return {"error": SUCCESS, "error_note": "Success"}
    ...
    def prepare(
        ...
        :return:
        """
        result = cls.click_webhook_errors(
            click_trans_id,
            service_id,
            merchant_trans_id,
            amount,
