...
class ApiHelper:
    ...
    def post(self, url: str, data: dict):
        response = requests.post(
            ...
            headers={
                ...
                "Auth": "{}:{}:{}".format(
                    self.merchant_user_id, self.token, self.timestamps
                ),
            },
        )
        return response
    ...
    def get_transaction(cls, transaction_id: int) -> Union[ClickTransaction, dict]:
        try:
            ...
            return {"error": -5001, "error_note": "Transaction not found"}
    ...
    def check_invoice(self):
        transaction = self.get_transaction(self.transaction_id)
        if isinstance(transaction, ClickTransaction):
            check_invoice = self.get(
                "/invoice/status/{service_id}/{invoice_id}".format(
                    service_id=self.service_id, invoice_id=self.data["invoice_id"]
                )
            )
    ...
    def check_payment_status(self):
        transaction = self.get_transaction(self.transaction_id)
        if isinstance(transaction, ClickTransaction):
            check_payment = self.get(
                "/payment/status/{service_id}/{payment_id}".format(
                    service_id=self.service_id, payment_id=transaction.click_paydoc_id
                )
            )
    ...
    def create_card_token(self):
        ...
        if isinstance(transaction, ClickTransaction):
            if transaction.status == ClickTransaction.CONFIRMED:
                return {"error": ALREADY_PAID, "error_note": "Transaction already paid"}
            data = {
                "service_id": self.service_id,
    ...
    def verify_card_token(self):
        ...
        if isinstance(transaction, ClickTransaction):
            if transaction.status != ClickTransaction.PROCESSING:
                return {"error": -5001, "error_note": "Transaction is not ready"}
            data = {
                "service_id": self.service_id,
    ...
    def delete_card_token(self):
        data = {"service_id": self.service_id, "card_token": self.data["card_token"]}
    ...
    def cancel_payment(self):
        ...
        if isinstance(transaction, ClickTransaction):
            ...
            response = self.delete(
                "/payment/reversal/{service_id}/{payment_id}".format(**data)
            )
            if response.status_code == 200:
                _json = response.json()
                if _json["error_code"] == 0:
...
class Services(ApiHelper):
    ...
    def api(self):
        if self.service_type == "create_invoice":
            ...
            return {"error": -1000, "error_note": "Service type could not be detected"}
